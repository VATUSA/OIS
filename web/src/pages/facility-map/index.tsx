...
export function FacilityMapPage() {
  ...
  return <FacilityMapView id={facilityId.toUpperCase()} />;
}

function FacilityMapView({ id }: { id: string | null }) {
  const { resolvedTheme } = useTheme();
  const navigate = useNavigate();

  const camera = useMapCamera(INITIAL);
  ...
  // Auto-center: frame the selected facility, or the whole CONUS on the national overview.
  useEffect(() => {
    const pts = feature ? facilityPoints(feature) : CONUS_BOUNDS;
    camera.fitBounds(pts, { padding: 40, maxZoom: feature ? 8 : 6 });
    // camera identity is stable; refit only when the facility changes.
  ...
  return (
    <div className="relative h-[calc(100vh-3.5rem)] w-full">
      ...
      >
        ...
        {/* Unknown facility id in the URL */}
        {id && !feature && (
