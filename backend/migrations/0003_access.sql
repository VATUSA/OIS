...
(
    ...
    permission_name text not null references access.permissions
(
    name
)
  on delete cascade,
...
(
    ...
    role_name text not null references access.roles
(
    name
)
  on delete cascade,
...
(
    ...
    service_account_id text not null references access.service_accounts
(
    id
) on delete cascade,
...
(
    ...
    user_id text references identity.users
(
    id
) on delete cascade,
    service_account_id text references access.service_accounts
(
    id
)
  on delete cascade,
...
(
    ...
    actor_id text references access.actors
(
    id
) on delete set null,
...
create
or replace view access.v_user_primary_role as
select ur.user_id,
       (
           array_agg(
                   ur.role_name order by
                case
...
create
or replace view access.v_effective_user_permissions as
