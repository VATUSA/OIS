...
-- outline-as-centerline mismatch), so they move over as that single ring. Every other row
-- (0067's KDCA OSM seed, manual/CRC-drawn centerlines) is an open line that can't become a polygon
-- without inventing a width, so it's deleted; the FAA seed covers KDCA and staff redraw by outline.

alter table flow.airport_taxiway add column if not exists rings jsonb;

update flow.airport_taxiway set rings = jsonb_build_array(points) where source = 'faa';
