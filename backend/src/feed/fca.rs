...
/// undercount the distance to the next named fix. The starting point for that accumulation is
/// whichever point `route_path`'s own logic treats as the start: the aircraft's current position
/// for an airborne aircraft (`gs >= 50`), or the first anchor (departure) otherwise — matching
/// what [`super::predict::eta_along_route`]'s `along_nm` parameter expects. Pass `gs = 0` for a
/// prefile (no live position) to always take the from-departure branch.
#[allow(clippy::too_many_arguments)]
