...
- **The web gates are `pnpm lint` and `pnpm typecheck`.** `pnpm lint` runs ESLint (root
  `eslint.config.mjs`) in `web` and `@ois/ui`: `react-hooks/rules-of-hooks` and unused variables are
  errors, `react-hooks/exhaustive-deps` is a warning. `pnpm typecheck` only tells the truth after the
  client is regenerated for any contract change (see codegen above).
